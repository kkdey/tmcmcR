...
for ( l in 1:L)
{
  Mult_Mattingly[1,l,,] <- tmcmcR:::mt_rwmh_metrop(pdf, base=base, scale=1, nmove=50, nsamples=1000, burn_in = NULL, verb=TRUE)$chain;
  Mult_Mattingly[2,l,,] <- tmcmcR:::mt_tmcmc_metrop(pdf, base=base, scale=1,nmove_size=5, nmove=50, nsamples=1000, burn_in = NULL, verb=TRUE)$chain;
  cat("We are at iter:",l, "\n")
}


KSval_mtry_RWMH=array(0,nsamples);
...
for(d in 1:30)
{
  for(n in 1:nsamples)
  {
    ...
    KSval_mtry_RWMH[n]=ks.test(Mult_Mattingly[1,,n,d],simulate.vec)$statistic;
    KSval_mtry_TMCMC[n]=ks.test(Mult_Mattingly[2,,n,d],simulate.vec)$statistic;

  }

  plot(1:nsamples,KSval_mtry_TMCMC,col="red",type="l",lwd=1,pch=2,xlab="",ylab="")
  lines(1:nsamples,KSval_mtry_RWMH,col="blue",lwd=1,pch=3)
  title(xlab="Time step of run");
  title(ylab="KS test distance");
  title(main="KS plot comparison");
  legend("topright",c("mtry_TMCMC","mtry_RWMH"),fill=c("red","blue"),border="black");
}
