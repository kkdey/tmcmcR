---
title: "tmcmcR:  R package for sampling with improved coverage"
author: "Kushal K Dey"
date: "`r Sys.Date()`"
...
```{r echo=TRUE, eval=TRUE}
system.time(out_rwmh <- rwmh_metrop(pdf,base=base, scale=1,nsamples=5000))
```

To run the TMCMC chain, use the `tmcmc_metrop` function.

```{r echo=TRUE, eval=TRUE}
system.time(out_tmcmc <- tmcmc_metrop(pdf,base=base, scale=1,nsamples=5000))
```
...
If the argument `verb` is TRUE, the function will output the progress of the simulation (how many runs of the samples have been completed). Also for the RAMA method, there are additional parameters `a_rama`, `b_rama` and `M_rama`. If these values are not provided by the user, the default value assumed are \begin{verb} a_rama =0 \end{verb}, \begin{verb} b_rama =0 \end{verb} and \begin{verb} M_rama =100 \end{verb}. 
...
Next we compare the performances of the adaptive versions of TMCMC against the test distribution and see how the three adaptive methods - due to Atchade, SCAM and the RAMA methods perform in comparison to each other. We plot for two randomly selected co-ordinates.

<img src='adaptive_tmcmc_compare.png' style="width:300px;height:200px;">
<img src='adaptive_tmcmc_compare_2.png' style="width:300px;height:200px;">
